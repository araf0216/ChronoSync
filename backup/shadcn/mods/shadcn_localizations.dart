...
class DefaultShadcnLocalizations extends ShadcnLocalizations {
  ...
  String formatTimeOfDay(TimeOfDay time,
    ...
    if (use24HourFormat) {
      ...
      int hour = (time.hour > 12 || time.hour == 0) ? (time.hour - 12).abs() : time.hour;
      if (showSeconds) {
        result +=
            '${hour.toString().padLeft(2, '0')}:${time.minute.toString().padLeft(2, '0')}:${time.second.toString().padLeft(2, '0')} ${time.hour >= 12 ? "PM" : "AM"}';
      } else {
        result +=
